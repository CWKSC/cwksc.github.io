...
1.  **情況 $\alpha \to \infty$ (無資訊/平坦先驗, Uninformative/Flat Prior)**：
    ...
    - _結果_：後驗平均數變成了標準的一般最小平方法 (OLS/MLE) 估計值。先驗失去了正則化 (regularizing) 的作用。
...
2.  **情況 $\alpha = 0$ (絕對的先驗信念, Absolute Prior Configuration)**：
    ...
    - _結果_：資料不再起任何作用。我們絕對確信 $\theta = 0$，完全不論觀察結果如何。
...
3.  **情況 $\sigma^2 \to 0$ (無雜訊觀察, Noise-free Observations)**：
    ...
    - _結果_：我們對「那些能完全擬合資料的參數」變得百分之百確定（後驗不確定性為零），前提是資料真的能被完美擬合。
