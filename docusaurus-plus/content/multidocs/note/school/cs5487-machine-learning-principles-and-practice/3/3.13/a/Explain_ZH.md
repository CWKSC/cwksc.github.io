...
- **脊迴歸 (Ridge Regression, L2)：** 假設權重服從 **高斯（常態）分佈 (Gaussian distribution)**。這個分佈看起來像一個平滑的山丘。在最高點（零）附近，它是相對平坦的。它告訴模型：「_接近零很好，但完全是零並沒有比 0.001 好多少。_」
- **LASSO (L1)：** 假設權重服從 **拉普拉斯分佈 (Laplace distribution)**。這個分佈看起來像一座陡峭的山，在零的位置有一個非常尖銳的峰值。它在對數尺度下線性下降，形成一個尖銳的突刺。它告訴模型：「_除非資料給你一個非常有說服力的理由，否則你應該完全是 0。_」
