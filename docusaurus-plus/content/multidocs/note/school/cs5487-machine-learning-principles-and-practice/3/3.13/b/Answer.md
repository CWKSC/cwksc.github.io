...
3. **Behavior at the Optimum:**
   ...
   Because our objective is to _minimize_ the cost function, any solution where both $\theta^+_i > 0$ and $\theta^-_i > 0$ simultaneously cannot be the true minimum. The optimizer will always prefer to subtract $c$ from both until at least one of them hits zero to strictly lower the objective value.
