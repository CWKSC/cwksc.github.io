...
There is no closed-form solution to (3.59), and hence an iterative method is needed to perform the optimization. Next, we will rewrite (3.59) into an equivalent _quadratic programming_ (QP) problem, which can be plugged into a standard solver (e.g., `quadprog` in MATLAB).
