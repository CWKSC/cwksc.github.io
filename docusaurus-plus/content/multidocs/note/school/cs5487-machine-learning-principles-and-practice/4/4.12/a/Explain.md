...
- **Forgetting that $\lambda$ is a shared constant**: In the step where we sum $\sum \frac{N_j}{\lambda} = 1$, remember that $\lambda$ has no $j$ subscript. It is the identical scaling factor applied to correct _every_ probability simultaneously.
