...
解決帶有等式約束的最佳化問題的其中一種方法是使用 _拉格朗日乘子 (Lagrange multipliers)_。考慮以下問題，
...
\_(圖片說明：這是一張等高線圖，顯示了目標函數 $f(x) = 1 - x_1^2 - x_2^2$ 的同心圓等位線 (level curves)。一條紅色直線代表約束函數 $g(x) = x_1 + x_2 - 1 = 0$。最佳解點 $(x_1^*, x_2^*) = (0.5, 0.5)$ 位於 $f(x)$ 的最高可能取值處且同時與約束線 $g(x)$ 相切的地方。一個指向中心的箭頭代表梯度 $\nabla f(x)$，它在最佳解點處正交於約束線。)\_
...
其中 $\lambda \neq 0$。定義 _拉格朗日函數 (Lagrangian function)_，
