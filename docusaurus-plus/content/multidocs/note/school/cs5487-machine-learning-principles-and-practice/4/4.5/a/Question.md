...
where $\lambda_j$ is the rate parameter for component $j$, and $\theta = \{\lambda_j, \pi_j\}_{j=1}^K$ the parameters of the mixture. Derive the EM algorithm to estimate the parameters of the model given samples $X = \{x_1, \cdots, x_n\}$. How is the M-step related to the ML estimate for a Poisson (Problem 2.1)?
