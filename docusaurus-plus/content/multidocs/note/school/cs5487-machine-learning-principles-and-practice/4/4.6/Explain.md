...
The problem is: when looking at the call logs (the data $x_i$), you only see the _duration_ of the call, but the system didn't record _which category_ (the latent variable $z_i$) the call belonged to!
...
- _If we guess the categories have certain average times and proportions..._
...
- **Local Optima:** The EM algorithm doesn't guarantee finding the _global_ best solution, only a _local_ one. Because of this, it is highly dependent on initialization. Running it multiple times with different starting parameters is a common practice.
